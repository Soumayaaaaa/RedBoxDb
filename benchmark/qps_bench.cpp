...
int main() {
    ...
    all_latencies.resize(num_threads * TIMED_QUERIES);
    ...
    std::cout << "Test single search...\n" << std::flush;
    (void)db->search(queries[0]);
    std::cout << "Single search done.\n" << std::flush;
...
threads.emplace_back([&, t]() {
    try {
        ...
        std::cout << "Thread " << t << " threw: " << e.what() << "\n" << std::flush;
    } catch (...) {
        std::cout << "Thread " << t << " threw unknown exception\n" << std::flush;
    }
});
    }

    std::cout << "Threads launched. Waiting...\n" << std::flush;
